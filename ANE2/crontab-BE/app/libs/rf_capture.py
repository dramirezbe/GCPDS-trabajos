...
def init_acquire_lib(library_path: str):
    ...
    try:
        ...
        log.error(f"Error loading shared library(acquire.so): {e}")
        raise
...
def acquire_signal(acquire_lib, start_freq_hz: float, end_freq_hz: float, resolution_hz: int, antenna_port: int) -> Optional[List[float]]:
    ...
    try:
        ...
        if not signal_ptr:
            log.error("Acquire_signal returned a NULL pointer (acquire.so)")
            return None
        ...
        if signal_ptr:
            try:
                ...
                log.warning(f"Failed to free C pointer: {e}")
