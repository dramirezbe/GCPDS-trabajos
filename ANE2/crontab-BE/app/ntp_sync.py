...
def set_system_time(dt_utc: datetime, sys_time: datetime, ntp_time: datetime):
    formatted_time = dt_utc.strftime("%Y-%m-%d %H:%M:%S")
    cmd = ["timedatectl", "set-time", formatted_time] if shutil.which("timedatectl") else ["date", "-u", "-s", formatted_time]
    if cfg.VERBOSE:
    ...
    if proc.returncode == 0:
        ...
        log.error(
            ...
            f"sys_time={sys_time.isoformat()} ntp_time={(ntp_time.isoformat() if ntp_time else 'None')} "
...
def main():
    ...
    try:
        ...
        if cfg.VERBOSE: log.info(f"System time (UTC): {sys_time.isoformat()}")
