...
#add Enhance the Model Architecture to the code

class ARNN(torch.nn.Module):
    """A flexible AR-NN: linear AR, or a deep MLP with dropout and activations.

    - If hidden_size == 0: A simple Linear(lags, 1) layer.
    - If hidden_size > 0: A multi-layer perceptron (MLP) with configurable
      depth, activation function, and dropout.
    """
    def __init__(
        self,
        lags: int,
        ...
        if hidden_size <= 0:
            # Fallback to simple linear model if no hidden layers are specified
        ...
        layers = []
        # --- Input Layer ---
        layers.append(torch.nn.Linear(lags, hidden_size, bias=True, dtype=dtype, device=DEVICE))
        ...
        for _ in range(num_hidden_layers - 1):
            layers.append(torch.nn.Linear(hidden_size, hidden_size, bias=True, dtype=dtype, device=DEVICE))
            layers.append(act_fn)
            if dropout_rate > 0:
                layers.append(torch.nn.Dropout(dropout_rate))

        # --- Output Layer ---
        layers.append(torch.nn.Linear(hidden_size, 1, bias=True, dtype=dtype, device=DEVICE))
...
def fit_ar_nn(
    ...
    lags: int = 10,
    samples_to_predict: int = 100,
    # --- New Architectural Hyperparameters ---
    hidden_size: int = 32,
    ...
    verbose: bool = True,
    *,
    include_bias: bool = False, # Note: bias is now True by default for MLPs
    ...
    Now supports creating deep MLPs with configurable layers, activations, and dropout.
    """
    if waveform.ndim != 2:
...
def process_audio_files(
    ...
    verbose: bool = True,
    # --- AR-NN options (UPDATED) ---
    use_nn_ar: bool = True,
    nn_hidden_size: int = 32,
    nn_num_hidden_layers: int = 2,      # <-- ADDED
    nn_activation_fn: str = 'relu',     # <-- ADDED
    nn_dropout_rate: float = 0.1,       # <-- ADDED
    nn_epochs: int = 5,
        ...
        try:
            ...
            if use_nn_ar:
                nn_state, nn_preds, nn_weights = fit_ar_nn(
                    ...
                    # --- Pass the new architectural parameters ---
                    hidden_size=nn_hidden_size,
                    num_hidden_layers=nn_num_hidden_layers,
                    activation_fn=nn_activation_fn,
                    dropout_rate=nn_dropout_rate,
                    # --- Pass the training parameters ---
                    epochs=nn_epochs,
                    ...
                    verbose=verbose,
                    include_bias=True, # Bias is generally useful for MLPs
                    use_exact_linear=(nn_hidden_size == 0), # Only use for pure linear
                    train_dtype=DTYPE,
                    max_windows_for_nn=None,
                    shuffle=True, # Shuffling is good practice for NN training
                )
                if nn_state is not None:
                    # Flatten all tensors from the model's state_dict for logging
                    flat_list = []
                    ...
                    # Log results and all key hyperparameters for traceability
                    item["nn_ar"] = {
                        "P": chosen_P,
                        "predictions": nn_preds,
                        "linear_weights_plus_bias": nn_weights, # Populated for linear models
                        "hidden_size": nn_hidden_size,
                        "num_hidden_layers": nn_num_hidden_layers, # <-- Logged
                        "activation_fn": nn_activation_fn,         # <-- Logged
                        "dropout_rate": nn_dropout_rate,           # <-- Logged
                        "state_dict_keys": list(nn_state.keys()),
                        "flat_params": flat_params, # Store all flattened model weights
                    }
                else:
                    # Explicitly mark as failed if NN training returned None
                    item["success"] = False
                    item["error"] = "NN-AR training failed"
            else:
                # If use_nn_ar is disabled, there's nothing to do in this pipeline.
                item["success"] = False
...
if __name__ == "__main__":
    ...
    base_path = "CommRad_Dataset/COBRAMR-678"  # or your path
    
    # Find all .wav files in the folder (limited to the first 5 for a quick test)
    audio_files = [os.path.basename(p) for p in glob.glob(os.path.join(base_path, "*.wav"))][:5]
    
    # Pre-filter: process ONLY the files that actually exist
    ...
    if not existing_files:
        print("❌ No valid audio files found to process in the base_path.")
    else:
        # --- Single call to the processing pipeline ---
        results = process_audio_files(
            existing_files,
            base_path=base_path,
            max_lags=20,                 # Increased lags for a more complex model
            samples_to_predict=128,
            target_sr=None,              # Use original sample rate
            mono_mode="first",
            max_duration_s=10.0,         # Process up to 10 seconds to speed up test
            verbose=True,
            
            # --- NN-AR parameters using new architectural features ---
            use_nn_ar=True,
            nn_hidden_size=64,           # Number of neurons per hidden layer
            nn_num_hidden_layers=2,      # Create a 2-hidden-layer network
            nn_activation_fn='gelu',     # Use GELU activation function
            nn_dropout_rate=0.15,        # Use 15% dropout for regularization
            nn_epochs=10,                # Increased epochs for the deeper model
            nn_batch_size=4096,          # Smaller batch size can sometimes help
            nn_lr=1e-3,
        ...
        # --- Save results and cluster the parameters ---
        ...
        if n_rows > 0:
            ...
            print("⏭️  Skipping clustering — no parameters were saved.")
