...
class TCNTrainer:
    ...
    def train(self, X_train, X_val, chunk_size, batch_size):
        ...
        for epoch in range(self.epochs):
            ...
            avg_val_snr = val_snr / len(val_loader)
            avg_val_psnr = val_psnr / len(val_loader)

            self.history['train_loss'].append(avg_train_loss)
            self.history['val_loss'].append(avg_val_loss)
            self.history['val_mae'].append(avg_val_mae)
            self.history['val_snr'].append(avg_val_snr)
            self.history['val_psnr'].append(avg_val_psnr)
    ...
    def evaluate(self, X_test, chunk_size):
        ...
        original = np.array(original_audios)
        reconstructed = np.array(reconstructed_audios)
    ...
    def plot_history(self):
        ...
        axs[0, 1].set_title('Validation MAE Over Epochs')
        ...
        axs[1, 0].set_title('Validation SNR Over Epochs')
        axs[1, 0].set_xlabel('Epochs')
        axs[1, 0].set_ylabel('Signal-to-Noise Ratio (dB)')
        axs[1, 0].legend()
        axs[1, 0].grid(True)

        # Plotting PSNR
        axs[1, 1].plot(self.history['val_psnr'], label='Validation PSNR', color='red')
        axs[1, 1].set_title('Validation PSNR Over Epochs')
        axs[1, 1].set_xlabel('Epochs')
        axs[1, 1].set_ylabel('Peak Signal-to-Noise Ratio (dB)')
        axs[1, 1].legend()
        axs[1, 1].grid(True)
...
if __name__ == '__main__':
    ...
    if audio_matrix.size == 0:
        ...
        tcn_model = TCNAutoencoder(
            input_channels=1, output_channels=1, num_channels=TCN_CHANNELS,
            kernel_size=TCN_KERNEL_SIZE, dropout=TCN_DROPOUT
        )

        trainer = TCNTrainer(model=tcn_model, device=device, lr=LEARNING_RATE, epochs=EPOCHS)
        
        # --- 4. Training ---
        trainer.train(X_train, X_val, chunk_size=CHUNK_SIZE, batch_size=BATCH_SIZE)
        trainer.plot_history()
        
        # --- 5. Evaluation ---
        performance_results = trainer.evaluate(X_test, chunk_size=CHUNK_SIZE)
