...
class TemporalBlock(torch.nn.Module): #Modified: Apply weight normalization on Convolutional layers.
    """
    A residual block for a TCN, with causal, dilated convolutions and weight normalization.
    """
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        super().__init__()
        self.net = torch.nn.Sequential(
            # Applying weight normalization to convolutional layers
            torch.nn.utils.parametrizations.weight_norm(torch.nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride,
                                                       padding=padding, dilation=dilation)),
            Chomp1d(padding),
            torch.nn.ReLU(),
            torch.nn.Dropout(dropout),
            torch.nn.utils.parametrizations.weight_norm(torch.nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride,
                                                       padding=padding, dilation=dilation)),
            Chomp1d(padding),
            torch.nn.ReLU(),
            torch.nn.Dropout(dropout)
        ...
        # Initialize downsample layer with weight normalization as well for consistency
        if self.downsample is not None:
            self.downsample = torch.nn.utils.parametrizations.weight_norm(self.downsample)
...
def fit_sequence_model( #Modified: Include data splitting, learning rate scheduling, and early stopping
    waveform: torch.Tensor,
    model_type: str = 'tcn',
    lags: int = 10,
    samples_to_predict: int = 100,
    nn_params: Optional[Dict[str, Any]] = None,
    epochs: int = 20,  # Increased default epochs for scheduler/early stopping
    batch_size: int = 8192,
    lr: float = 1e-3,
    weight_decay: float = 1e-4, # Added slightly more weight decay
    grad_clip: float = 1.0,
    verbose: bool = True,
    shuffle: bool = True,
    ...
    """
    Train ARNN/TCN with validation, learning rate scheduling, and early stopping.
    """
    ...
    if mtype != 'tcn':
        # Fallback to the original function for ARNN to keep it simple
        # Or you can integrate the same logic for ARNN as well
        print("Using legacy trainer for ARNN model.")
        # (You would call your original `fit_sequence_model` here if you've renamed it)
        # For this example, we'll just error out.
        raise ValueError("This optimized trainer is configured for TCN. ARNN support needs to be added.")

    # --- Build and prepare TCN Model ---
    if verbose: print("Initializing TCNModel...")
    model = TCNModel(
        num_channels=nn_params.get("channels", [16, 32, 64]), # Default deeper arch
    ...
    indices = torch.randperm(M, device=DEVICE) if shuffle else torch.arange(M, device=DEVICE)
    split_idx = int(M * (1 - validation_split))
    train_indices, val_indices = indices[:split_idx], indices[split_idx:]

    if len(val_indices) == 0:
        if verbose: print("Warning: No validation samples after split. Using train set for validation.")
        val_indices = train_indices

    # --- Setup Training Components ---
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    loss_fn = torch.nn.MSELoss()
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, 'min', factor=0.2, patience=lr_patience)
    ...
    for ep in range(1, epochs + 1):
        ...
        if verbose:
            print(f"Epoch {ep}/{epochs}, Train Loss: {avg_train_loss:.6f}, Val Loss: {avg_val_loss:.6f}")
    ...
    if best_model_state is None:
        # This happens if training ends on the first epoch
        best_model_state = model.state_dict()
    ...
    preds_future = _roll_predict_sequence(
        model, last_context=last_ctx, steps=samples_to_predict, flip_input=False
    )

    return best_model_state, preds_future

# ===== Orchestrator =====
def process_audio_files( # Modified: added params of fit_sequence_model modification in this pipeline function
    audio_files: list,
    ...
    # --- Add the new parameters here with default values ---
    validation_split: float = 0.2,
    lr_patience: int = 3,
    early_stopping_patience: int = 5,
) -> Dict[str, Any]:
    ...
    for i, file_name in enumerate(audio_files, 1):
        ...
        try:
            ...
            # --- Pass the new parameters to fit_sequence_model ---
...
if __name__ == "__main__":
    ...
    base_path = Path("/home/javastral/GIT/GCPDS--trabajos-/Audios-AR/PresenceANEAudios")  # Adjust to your audio folder
    
    model_to_run = 'tcn'
    
    if not base_path.exists():
        print(f"❌ Base path does not exist: {base_path}")
        # Create a dummy folder and file for demonstration if it doesn't exist
        base_path.mkdir(exist_ok=True)
        # (Code to create a dummy wav file would go here if needed for testing)
    
    audio_files = [p.name for p in base_path.glob("*.wav")]
    if not audio_files:
        print(f"❌ No .wav files found in {base_path}. Please add some audio files.")
    else:
        # --- Optimized TCN Configuration ---
        if model_to_run == 'tcn':
            print("--- Configuring Optimized TCN Model (Deeper) ---")
            nn_config = {
                "model_type": 'tcn',
                "max_lags": 128,  # Increased receptive field
                "nn_params": {
                ...
                "nn_epochs": 5, # Higher epoch count to allow for early stopping
                "nn_batch_size": 1024,
                "nn_lr": 1e-3,
                "nn_weight_decay": 1e-4,
            }
            # Add new training params to the function call
            training_params = {
        ...
        else: # Add your ARNN config here if needed
            raise ValueError("This script is currently configured to run the optimized TCN.")
        ...
        results = process_audio_files(
            ...
            max_duration_s=10.0,
            **nn_config,
            **training_params # Pass the new training params here
        )
